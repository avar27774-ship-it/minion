...
export default function ProfilePage() {
  ...
  const isOwnProfile = !id || id === me?._id || id === me?.id
    ...
    <div style={{ maxWidth:900, margin:'0 auto', padding:'40px 20px' }}>
      <div style={{ display:'grid', gridTemplateColumns:'280px 1fr', gap:24 }}>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
          {Array(3).fill(0).map((_,i) => <div key={i} className="skel" style={{ height:80 }}/>)}
        </div>
      </div>
    </div>
  )
  ...
  const joinDate = new Date((profile.created_at || profile.createdAt) * 1000 || profile.createdAt)
      ...
      <div style={{ display:'grid', gridTemplateColumns:'300px 1fr', gap:24, alignItems:'start' }}>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
          ...
          <div style={{
            ...
            <div style={{ color:'var(--t3)', fontSize:13, marginBottom:8 }}>
          ...
          {isOwnProfile && (
            <Link to="/wallet" className="btn btn-secondary" style={{ textAlign:'center' }}>
              💰 Кошелёк
            </Link>
          )}
        </div>
                    ...
                    <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:10 }}>
                      ...
                      <div style={{ flex:1 }}>
                        ...
                        <div style={{ fontSize:11, color:'var(--t4)' }}>
                          {r.product_title && `Товар: ${r.product_title} · `}
                          {new Date(r.created_at * 1000 || r.createdAt).toLocaleDateString('ru')}
                        </div>
                      </div>
