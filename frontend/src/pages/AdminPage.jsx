...
const STATUS_COLOR = { active:'var(--green)', pending:'var(--accent)', completed:'var(--t3)', disputed:'var(--red)', refunded:'#22d3ee', cancelled:'var(--t4)' }
const STATUS_LABEL = { active:'Активна', pending:'Ожидание', completed:'Завершена', disputed:'Спор', refunded:'Возврат', cancelled:'Отменена' }
...
export default function AdminPage() {
  ...
  const adjustBalance = async (id) => {
    ...
    res.ok ? (toast.success(`Баланс изменён. Новый: $${res.newBalance?.toFixed(2)}`), loadTab('users')) : toast.error(res.error)
  }
    ...
    <div style={{ maxWidth:1280, margin:'0 auto', padding:'24px 20px', minHeight:'100vh' }}>
      ...
      <div style={{ display:'flex', gap:6, marginBottom:24, flexWrap:'wrap' }}>
        {TABS.map(([v, l]) => (
          <button key={v} onClick={() => setTab(v)} style={{
            ...
            fontFamily:'var(--font-h)', transition:'all 0.15s',
        ...
        <div>
          <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:16, marginBottom:28 }}>
            {[
              ...
              ['Доход (комиссии)', '$' + parseFloat(stats.revenue || 0).toFixed(2), '💰']
            ].map(([l, v, i]) => (
      ...
      {tab === 'users' && !loading && (
        <div>
          <div style={{ display:'flex', gap:10, marginBottom:16 }}>
            <input className="inp" placeholder="Поиск по логину или TG ID..." value={userSearch} onChange={e => setUserSearch(e.target.value)} style={{ flex:1 }}
              ...
              <div key={u._id || u.id} style={{
                ...
                <div style={{ flex:1 }}>
                  <div style={{ fontWeight:600, fontSize:14 }}>
                    @{u.username || '—'}
                    {u.isAdmin && <span style={{ color:'var(--accent)', marginLeft:6 }}>⚡Admin</span>}
                    {u.isVerified && <span style={{ color:'var(--green)', marginLeft:6 }}>✓</span>}
                    {u.isBanned && <span style={{ color:'var(--red)', marginLeft:6 }}>🚫Бан</span>}
                  </div>
                  <div style={{ fontSize:12, color:'var(--t3)' }}>
                    Баланс: ${parseFloat(u.balance || 0).toFixed(2)} · Сделок: {(u.totalSales || 0) + (u.totalPurchases || 0)} · Рейт: {(u.rating || 5).toFixed(1)}
                  </div>
                  {u.telegram_id && <div style={{ fontSize:11, color:'var(--t4)' }}>TG: {u.telegram_id}</div>}
                </div>
                <div style={{ display:'flex', gap:6, flexWrap:'wrap' }}>
                  <button className="btn btn-sm btn-secondary" onClick={() => adjustBalance(u._id || u.id)} title="Баланс">💰</button>
                  {!u.isVerified && <button className="btn btn-sm btn-secondary" onClick={() => verifyUser(u._id || u.id)} title="Верифицировать">✓</button>}
                  {u.isBanned
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
          {deals.filter(d => d.status === 'disputed').length > 0 && (
            <div style={{ background:'rgba(231,76,60,0.06)', border:'1px solid rgba(231,76,60,0.3)', borderRadius:12, padding:'10px 16px', marginBottom:8, fontSize:13, color:'var(--red)', fontWeight:700 }}>
              ⚠️ {deals.filter(d => d.status === 'disputed').length} активных спора требуют рассмотрения
            </div>
          )}
          {deals.map(d => (
          ...
          {deals.length === 0 && <div style={{ textAlign:'center', padding:'40px', color:'var(--t3)' }}>Сделок нет</div>}
        </div>
      )}
      ...
      {tab === 'products' && !loading && (
        <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
          ...
          {products.length === 0 && <div style={{ textAlign:'center', padding:'40px', color:'var(--t3)' }}>Товаров нет</div>}
        </div>
      )}
              ...
              <div key={tx._id || tx.id} style={{ background:'var(--bg2)', border:'1px solid var(--border)', borderRadius:12, padding:'12px 16px', display:'flex', alignItems:'center', gap:12 }}>
                <div style={{ flex:1 }}>
                  ...
                  <div style={{ fontSize:11, color:'var(--t3)' }}>
                    ...
                    {tx.gateway_type && ` · ${tx.gateway_type}`}
                  </div>
                </div>
        ...
        <div style={{ maxWidth:500 }}>
          ...
          <div style={{ background:'var(--bg2)', border:'1px solid var(--border)', borderRadius:16, padding:24 }}>
            <div style={{ marginBottom:12 }}>
              ...
              <input className="inp" placeholder="UUID пользователя" value={msgUserId} onChange={e => setMsgUserId(e.target.value)}/>
            </div>
            <div style={{ marginBottom:16 }}>
              ...
              <textarea className="inp" rows={4} placeholder="Сообщение..." value={msgText} onChange={e => setMsgText(e.target.value)} style={{ resize:'vertical' }}/>
            </div>
            <button className="btn btn-primary btn-full" onClick={sendMessage}>📤 Отправить в Telegram</button>
            <div style={{ marginTop:12, fontSize:12, color:'var(--t4)', lineHeight:1.6 }}>
              💡 Получить ID пользователя можно на вкладке «Пользователи»
            </div>
          </div>
        </div>
      )}
