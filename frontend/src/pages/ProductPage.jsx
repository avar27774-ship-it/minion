...
export default function ProductPage() {
  ...
  useMeta(product ? {
    title: product.title,
    description: `${product.title}. Безопасная покупка через гарант на Minions Market.`,
  ...
  const buy = async () => {
    if (!user) return navigate('/auth')
    if (!window.confirm(`Купить "${product.title}" за ${fmt(product.price)}? Средства будут заморожены до подтверждения получения.`)) return
    setBuying(true)
    try {
            ...
            <div style={{ marginBottom:16 }}>
              <div style={{ display:'flex', gap:10, marginBottom:10 }}>
                <button className="btn btn-primary" onClick={buy} disabled={buying} style={{ flex:1, padding:'16px', fontSize:16 }}>
                  {buying ? 'Создание сделки...' : `Купить за ${fmt(product.price)}`}
                </button>
                <button
