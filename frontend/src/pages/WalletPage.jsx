...
const TX_ICONS  = { deposit:'↓', withdrawal:'↑', commission:'%', purchase:'🛒', sale:'💸', refund:'↩', adjustment:'⚡', freeze:'🔒' }
const TX_COLORS = { deposit:'var(--green)', withdrawal:'var(--red)', sale:'var(--green)', refund:'#22d3ee', adjustment:'var(--purple)', purchase:'var(--red)', freeze:'var(--t3)' }
...
export default function WalletPage() {
  ...
  const withdraw = async () => {
    ...
    if (!address.trim()) return toast.error('Введите адрес / @username CryptoBot')
    setWorking(true)
    try {
  ...
  return (
    <div style={{ maxWidth:700, margin:'0 auto', padding:'32px 20px' }}>
      ...
      <div style={{
        ...
        <div style={{ fontFamily:'var(--font-h)', fontWeight:800, fontSize:48, color:'var(--accent)', marginBottom:4 }}>
          ${bal.toFixed(2)}
        </div>
        {frz > 0 && <div style={{ color:'var(--t3)', fontSize:13, marginBottom:16 }}>🔒 В сделках: ${frz.toFixed(2)}</div>}
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:12, marginBottom:28 }}>
        {[
          ...
          <div key={l} style={{ background:'var(--bg2)', border:'1px solid var(--border)', borderRadius:14, padding:'16px', textAlign:'center' }}>
      ...
      {loading ? (
        <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
          {Array(4).fill(0).map((_,i) => <div key={i} className="skel" style={{ height:64 }}/>)}
        </div>
      ) : txs.length === 0 ? (
      ...
      {modal === 'deposit' && (
        <Modal onClose={() => setModal(null)}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 2fr', gap:10 }}>
            ...
            <button className="btn btn-primary" onClick={deposit} disabled={working}>
      ...
      {modal === 'withdraw' && (
        <Modal onClose={() => setModal(null)}>
          ...
          <div style={{ fontSize:13, color:'var(--t3)', marginBottom:20, lineHeight:1.6 }}>
            Вывод осуществляется через <b style={{ color:'var(--t2)' }}>CryptoBot</b> в USDT.<br/>
            Минимальная сумма — $5. Обработка до 24ч.
          </div>
          ...
          <div style={{ background:'rgba(245,200,66,0.06)', border:'1px solid rgba(245,200,66,0.2)', borderRadius:12, padding:12, marginBottom:16, fontSize:12, color:'var(--t3)', lineHeight:1.6 }}>
            💡 Откройте <a href="https://t.me/CryptoBot" target="_blank" rel="noopener" style={{ color:'var(--accent)' }}>@CryptoBot</a> в Telegram, найдите свой адрес USDT и вставьте ниже.
          </div>

          <div style={{ display:'grid', gridTemplateColumns:'1fr 2fr', gap:10 }}>
            <button className="btn btn-ghost" onClick={() => setModal(null)}>Отмена</button>
            <button className="btn btn-primary" onClick={withdraw} disabled={working}>
              {working ? <Spinner /> : '↑ Вывести'}
            </button>
          </div>
        </Modal>
      )}
