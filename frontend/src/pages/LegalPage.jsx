...
import { FileText, Lock, RotateCcw, Mail, Truck, BookOpen, Shield } from '../components/Icon'
...
const PAGES = {
  ...
  refund: {
    ...
    sections: [
      ...
      },
      {
        ...
        content: `Если у вас возникли вопросы по статусу заявки на возврат или вы не можете открыть спор через интерфейс — обратитесь в поддержку: Telegram @for_ewer0721 или email anvarikromov778@gmail.com. Время ответа — до 2 часов.`,
      },
    ],
  },
  offer: {
  ...
  moderation: {
    ...
    sections: [
      {
        heading: '1. Общие положения',
        content: `Все товары, размещаемые на платформе Minions.Market, проходят обязательную проверку (модерацию) перед публикацией в каталоге. Цель модерации — защита покупателей от недобросовестных продавцов, исключение незаконного контента и обеспечение соответствия товаров правилам платформы и требованиям действующего законодательства.`,
      },
      {
        heading: '2. Этапы проверки товара',
        items: [
          'Шаг 1. Продавец создаёт объявление: заполняет название, описание, категорию, цену и прикладывает скриншоты товара.',
          'Шаг 2. Объявление поступает в очередь модерации со статусом «На проверке» и не отображается в каталоге.',
          'Шаг 3. Модератор (сотрудник администрации или автоматическая система) проверяет соответствие объявления правилам платформы.',
          'Шаг 4. Объявление получает статус «Опубликовано» либо отклоняется с указанием причины.',
          'Шаг 5. В случае отклонения продавец получает уведомление в Telegram и может исправить объявление для повторной проверки.',
        ],
      },
      {
        heading: '3. Что проверяется при модерации',
        items: [
          'Соответствие описания и категории реальному товару.',
          'Наличие достаточного количества информации для принятия решения о покупке.',
          'Отсутствие признаков мошенничества: краденые аккаунты, поддельные товары.',
          'Соответствие цены заявленным характеристикам товара (явно заниженная цена — признак подозрительного лота).',
          'Отсутствие запрещённого контента (см. раздел 4).',
          'Корректность скриншотов: они должны подтверждать описание и принадлежность товара продавцу.',
        ],
      },
      {
        heading: '4. Запрещённые к размещению товары',
        items: [
          'Аккаунты и предметы, полученные мошенническим путём или кражей.',
          'Читы, боты, программы для взлома игр и эксплойты.',
          'Персональные данные третьих лиц (базы данных, e-mail листы и т.п.).',
          'Контент, нарушающий авторские права без разрешения правообладателя.',
          'Любые товары, оборот которых запрещён законодательством.',
          'Вводящие в заблуждение объявления с ложным описанием характеристик.',
        ],
      },
      {
        heading: '5. Сроки модерации',
        content: `В стандартном режиме объявления проверяются в течение 2–12 часов с момента подачи. В периоды высокой нагрузки срок может составлять до 24 часов. Продавец получает уведомление в Telegram сразу после принятия решения. Автоматические фильтры обрабатывают явно запрещённый контент без участия модератора.`,
      },
      {
        heading: '6. Повторная проверка и обжалование',
        items: [
          'При отклонении объявления продавец получает конкретную причину и рекомендации по исправлению.',
          'После внесения правок продавец вправе отправить объявление на повторную модерацию.',
          'Если продавец считает решение ошибочным — он может обратиться в поддержку: Telegram @for_ewer0721.',
          'Апелляция рассматривается в течение 24 часов другим сотрудником администрации.',
          'При систематических нарушениях (3 и более отклонённых объявления с запрещённым контентом) аккаунт продавца блокируется.',
        ],
      },
      {
        heading: '7. Мониторинг активных объявлений',
        content: `Модерация не ограничивается проверкой при публикации. Администрация проводит регулярный мониторинг активных объявлений и вправе снять любое объявление с публикации без предварительного уведомления, если оно перестало соответствовать правилам. Покупатели и продавцы также могут пожаловаться на объявление — жалоба рассматривается в течение 24 часов.`,
      },
      {
        heading: '8. Ответственность продавца',
        content: `Продавец несёт полную ответственность за достоверность размещённых сведений. Прохождение модерации не снимает с продавца ответственности за качество и законность товара. В случае поступления жалоб от покупателей после публикации объявление может быть снято, а средства заморожены до выяснения обстоятельств.`,
      },
    ],
  },
  ...
  contacts: {
    ...
    contacts: [
      ...
      },
      {
        icon: '📞',
        label: 'Телефон',
        value: '+998 99 103 05 10',
        href: 'tel:+998991030510',
        desc: 'Служба поддержки',
      },
      {
        icon: '📍',
        label: 'Адрес',
        value: 'Узбекистан, г. Бухара, ул. Пиридастгир, д. 7',
        href: null,
